...
private fun socialLinks(row:Boolean = false){
    Link(
        ...
        FaFacebook(modifier = SocialLinksStyle
            .toModifier()
            .margin(
                bottom = if(row) 0.px else 40.px,
                right = if(row) 40.px else 0.px), size = IconSize.LG)
    }
    Link(
        path = TWITTER_LINK,
        openExternalLinksStrategy = OpenLinkStrategy.IN_NEW_TAB){
        FaTwitter(modifier = SocialLinksStyle
            .toModifier()
            .margin(
                bottom = if(row) 0.px else 40.px,
    ...
    Link(
        path = INSTAGRAM_LINK,
        openExternalLinksStrategy = OpenLinkStrategy.IN_NEW_TAB){
        FaInstagram(modifier = SocialLinksStyle
            .toModifier()
            .margin(
                bottom = if(row) 0.px else 40.px,
                right = if(row) 40.px else 0.px),
    ...
    Link(
        path = LINKEDIN_LINK,
        openExternalLinksStrategy = OpenLinkStrategy.IN_NEW_TAB){
        FaLinkedin(modifier = SocialLinksStyle
